...
Model::Model(const ModelSetting& model_setting, mobilint::Accelerator& acc)
    : mInputType(model_setting.input_type),
      mDevNo(model_setting.dev_no),
      mMxqPath(model_setting.mxq_path) {
    ...
    const auto& in = mModel->getInputBufferInfo()[0];
    std::cerr << "[Model] dev=" << mDevNo << " mxq=" << mMxqPath
              << " configured_input=float32"
              << " input_shape=" << in.original_width << "x" << in.original_height << "x"
              << in.original_channel << std::endl;
